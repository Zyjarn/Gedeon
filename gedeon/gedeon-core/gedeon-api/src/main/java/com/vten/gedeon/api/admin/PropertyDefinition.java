...
public interface PropertyDefinition extends PersistableObject {
	...
	public default Boolean isRequired() {
		return getProperties().get(GedeonProperties.PROP_IS_REQUIRED).getBooleanValue();
	}
	...
	/**
	 * Set the id of the associated property template - must be a valid entry
	 * template id for save
	 * 
	 * @param propTpltId
	 */
	...
	public default String getPropertyName() {
		return getPropertyTemplate().getName();
	}
	...
	public default boolean isList() {
		return getPropertyTemplate().isList();
	}

	public default GedId getAssociatedClassId() {
		return getProperties().get(GedeonProperties.PROP_ASSOCIATED_CLASS_ID).getIdValue();
	}

	public default void setAssociatedClassId(GedId defaultValue) {
		setPropertyValue(GedeonProperties.PROP_ASSOCIATED_CLASS_ID, defaultValue);
	}

	/**
	 * Get the class definition for which the current instance of property
	 * definition belong
	 * 
